## LightPhono v0.8 — The wheel moved out
...
This does mean a clean checkout needs credentials again. GitHub Packages has no anonymous read
even for public packages, so `gpr.user` / `gpr.key` are back in `local.properties` — the same
property names upstream phono used for the private keyboard artifact that is no longer here.
