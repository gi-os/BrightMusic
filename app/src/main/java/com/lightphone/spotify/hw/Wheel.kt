package com.lightphone.spotify.hw
...
/**
 * Whether the wheel reaches the scrollers in this subtree.
 *
 * Sibling apps pass `active` to each `WheelScroll` by hand, because a screen there usually knows
 * whether it is the visible one. This shell does not work that way: `PhonoShell` composes the
 * current tab *and* the overlay `NavHost` at once, and the context menu and its confirmations are
 * Boxes in the same window drawn over both. So the answer belongs to the shell, which is the only
 * thing that knows which layer is on top, and it is handed down rather than threaded through
 * fourteen screen signatures.
 *
 * Deliberately a plain [compositionLocalOf] and not a static one: a static local invalidates its
 * whole subtree when the value changes, which here would recompose an entire library list every
 * time an overlay opened. This one is read in exactly one place — [ArmedNotches] — so only those
 * few composables restart.
 */
private val LocalWheelActive = compositionLocalOf { true }

/**
 * Turn the wheel on or off for everything inside.
 *
 * Nests: an inner `false` wins over an outer `true` and vice versa, because each provider replaces
 * the value for its own subtree. Wraps composition only — no layout node, so it cannot affect
 * measurement.
 */
@Composable
...
/** The same, for the OAuth WebViews, which Compose knows nothing about. */
@Composable
...
/*
 * There is no `WheelInDialog` here, and no dialog-window walk to go with it.
 *
 * A Compose `Dialog` or a Material `ModalBottomSheet` is a window of its own, so keys never reach
 * the activity's `dispatchKeyEvent` while one is up, and sibling apps have to borrow that window's
 * callback. This app has none: the context menu, the copy toast and the delete confirmations are
 * all Boxes drawn over the shell in the activity's own window, so they already get the wheel. What
 * they need is the opposite — for the list underneath to stop answering — which is [WheelGate].
 */
